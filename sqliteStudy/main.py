...
def print_db():
    try:
        result = the_cursor.execute("SELECT id, f_name, l_name, age, address, salary, hire_data FROM employees")
        for row in result:
            print("id:", row[0])
            print("f_name:", row[1])
            print("l_name:", row[2])
...
print("SQLITE_VERSION :", the_cursor.fetchone())
